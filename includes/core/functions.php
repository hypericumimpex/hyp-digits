...
function getTranslatedCountryName( $countryName ) {
	$data = array(
		...
		"Côte d'Ivoire"                    => __( "Côte d'Ivoire", "digits" ),
...
function digCountry() {
	...
	foreach ( $countryList as $key => $value ) {
		...
		$valCon .= '<li class="dig - cc - visible ' . $ac . '" value="' . $value . '" country="' . strtolower( $key ) . '">(+' . $value . ') ' . getTranslatedCountryName( $key ) . '</li>';
	}

	echo '<ul class="digit_cs-list" style="display: none;">' . $valCon . '</ul>';
}
